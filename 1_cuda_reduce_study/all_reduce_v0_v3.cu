...
__global__ void all_reduce_v0_global_memory(float *d_input, float *d_output, int N)
{
    ...
    {
        bool valid = (idx + i) < N;
        if (valid && threadIdx.x % (i * 2) == 0)
        {
            input_begin[threadIdx.x] += input_begin[threadIdx.x + i];
        }
        // 这里的__syncthreads()很重要！！
        __syncthreads();
    }
    if (threadIdx.x == 0 && blockIdx.x * blockDim.x < N)
        d_output[blockIdx.x] = input_begin[0];
}
__global__ void all_reduce_v1_shared_memory(float *d_input, float *d_output, int N)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void all_reduce_v2_no_divergence_branch(float *d_input, float *d_output, int N)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    __shared__ float sdata[THREAD_PER_BLOCK];
    sdata[threadIdx.x] = (idx < N) ? d_input[idx] : 0.0f;
    ...
    for (int i = 1; i < blockDim.x; i *= 2)
    {
        ...
        {
            sdata[2 * i * threadIdx.x] += sdata[2 * i * threadIdx.x + i];
        }
        __syncthreads();
    }
    if (threadIdx.x == 0 && blockIdx.x * blockDim.x < N)
...
__global__ void all_reduce_v2_no_divergence_branch_1(float *d_input, float *d_output, int N)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    __shared__ float sdata[THREAD_PER_BLOCK];
    sdata[threadIdx.x] = (idx < N) ? d_input[idx] : 0.0f;
    ...
    for (int i = 1; i < blockDim.x; i *= 2)
    {
        if (threadIdx.x < blockDim.x / (2 * i))
        {
            sdata[threadIdx.x] = sdata[2 * threadIdx.x] + sdata[2 * threadIdx.x + 1];
        }
        __syncthreads();
    }
    if (threadIdx.x == 0 && blockIdx.x * blockDim.x < N)
...
__global__ void all_reduce_v3_no_bank_conflict(float *d_input, float *d_output, int N)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    __shared__ float sdata[THREAD_PER_BLOCK];
    sdata[threadIdx.x] = (idx < N) ? d_input[idx] : 0.0f;
    ...
    for (int i = 1; i < blockDim.x; i *= 2)
    {
        if (threadIdx.x < blockDim.x / (2 * i))
        {
            sdata[threadIdx.x] += sdata[threadIdx.x + blockDim.x / (2 * i)];
        }
        __syncthreads();
    }
    if (threadIdx.x == 0 && blockIdx.x * blockDim.x < N)
...
__global__ void all_reduce_v3_no_bank_conflict_1(float *d_input, float *d_output, int N)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    __shared__ float sdata[THREAD_PER_BLOCK];
    sdata[threadIdx.x] = (idx < N) ? d_input[idx] : 0.0f;
...
int main()
{
    ...
    all_reduce_v1_shared_memory<<<Grid, Block>>>(d_input, d_output, N);
