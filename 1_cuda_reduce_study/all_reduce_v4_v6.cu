...
__global__ void all_reduce_v4_add_during_load_plan_a(float *d_input, float *d_output, int N)
{
...
__global__ void all_reduce_v5_unroll_last_wrap(float *d_input, float *d_output, int N)
{
    int idx = blockIdx.x * blockDim.x * 2 + threadIdx.x;
    __shared__ float sdata[THREAD_PER_BLOCK];
    sdata[threadIdx.x] = (idx < N) ? d_input[idx] : 0.0f;
...
__global__ void all_reduce_v6_completely_unroll(float *d_input, float *d_output, int N)
{
    int idx = blockIdx.x * blockDim.x * 2 + threadIdx.x;
    __shared__ float sdata[THREAD_PER_BLOCK];
    sdata[threadIdx.x] = (idx < N) ? d_input[idx] : 0.0f;
...
int main()
{
    ...
    int block_num = (N + 2 * THREAD_PER_BLOCK - 1) / (THREAD_PER_BLOCK * 2);
    // 准备内存空间
    ...
    for (int i = 0; i < block_num; i++)
    {
        ...
        {
            if (i * 2 * THREAD_PER_BLOCK + j < N)
                cur += h_input[i * 2 * THREAD_PER_BLOCK + j];
        }
        h_output_cpu[i] = cur;
    }
    ...
    all_reduce_v6_completely_unroll<<<Grid, Block>>>(d_input, d_output, N);
